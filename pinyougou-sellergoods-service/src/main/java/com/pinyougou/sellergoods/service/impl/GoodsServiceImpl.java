...
public class GoodsServiceImpl implements GoodsService {
	...
	private void saveItemList(Goods goods) {
		// 是否启用规格进行判断
		if (GoodsEnums.ENABLE_SPEC.getStatus().equals(goods.getGoods().getIsEnableSpec())) {
			for (TbItem item : goods.getItemList()) {
				// 构建标题  SPU名称 + 规格选项
				StringBuffer title = new StringBuffer(goods.getGoods().getGoodsName());
			...
			// 不启用规格时，需要自己new 一个item，进行设置值
			TbItem item = new TbItem();
			item.setTitle(goods.getGoods().getGoodsName());
			item.setPrice(goods.getGoods().getPrice());
			item.setNum(99999); // 默认九九九
			item.setStatus(GoodsEnums.NORMAL_STATUS.getStatus()); // 默认状态1
			item.setSpec("{}");
			setItemValue(goods, item);
			itemMapper.insert(item);
		}
	...
	public PageResult findPage(TbGoods goods, int pageNum, int pageSize) {
		...
		criteria.andIsDeleteIsNull(); //指定条件未逻辑删除记录
